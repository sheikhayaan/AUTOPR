  ...
  for (const raw of lines) {
    ...
    if (fence) {
      if (line.trim().startsWith('```')) {
        blocks.push(<CodeBlock key={`code${blocks.length}`} lang={fence.lang} lines={fence.lines} />)
        fence = null
      } else {
    ...
    if (summary) {
      ...
      flushList()
      blocks.push(
        <p key={`s${blocks.length}`} className="mt-3 text-xs font-semibold uppercase tracking-wide text-ink-faint">
          {summary[1]}
        </p>,
      )
      continue
...
export default function CommentPreview({ body }) {
  return (
