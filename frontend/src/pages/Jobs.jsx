...
import { shortSha, relativeTime, absTime, statusMeta, pluralize } from '../lib/format.js'

const FILTERS = ['all', 'processing', 'queued', 'pending', 'done', 'dead']

function JobRow({ job }) {
  const prUrl = `https://github.com/${job.repo}/pull/${job.pr_number}`
  ...
  return (
    <div className="grid grid-cols-1 items-center gap-3 rounded-xl border border-line bg-surface/60 px-4 py-3.5 transition-colors hover:bg-surface-2/60 md:grid-cols-[minmax(0,1fr)_120px_84px_minmax(0,1.3fr)_96px]">
      ...
      <div className="text-xs text-ink-dim md:text-center">
        ...
        <span className={`tnum font-medium ${job.attempts > 1 ? 'text-risk-medium' : 'text-ink-dim'}`}>
          {job.attempts ?? 0}
        </span>
      </div>
      ...
      {/* Updated */}
      <div className="text-xs text-ink-faint md:text-right" title={absTime(job.updated_at || job.created_at)}>
...
export default function Jobs() {
  const { data, error, loading, refreshing, updatedAt, reload } = usePolling(() => api.jobs(100), 5000)
      ...
      {visible.length === 0 ? (
        <div className="mt-4 card">
          <EmptyState
            ...
            title={filter === 'all' ? 'No jobs yet' : `No ${statusMeta(filter).label.toLowerCase()} jobs`}
            description={
              filter === 'all'
